...
friesSelected=False
orderInitiation=input("would you like to order (y or n) ")
while(orderInitiation!="n"):
     ...
     if editOrder=="y":
          partToChange=input("What part would you like to change, entree(e), beverage(b), side(s), ketchup(k)? ")
          if partToChange=="e":
               replacement=input("What would you like to change it to.\na Krabby patty(k) for $1.25, a Double Krabby patty(dk) for $2.00, a Triple Krabby patty(tk) for $3.00, a Salty Sea Dog(d) for $1.25, a Footlong(f) for $2.00, a Sailors Suprise(s) for $3.00, or the GOLDEN LOAF(g) for $2.00.")
               order[0]=replacement
               if replacement=="k":
                    finalOrderList[1]="Krabby Patty"
               elif replacement=="dk":
     ...
     if deleteOrder=="y":
          numToRemove=int(input("What was the order number? "))
          orderList.remove(orderList[numToRemove-1])
          finalOrderListList.remove(finalOrderList[numToRemove-1])
